#################################################
######## Spatial Coupling 3D AI Para Est ########
#################################################

#%%# Catalyzer

sup_comp = True # Super Computer?
opt = 0 # Careful Usage! The function 'construct_stem' utilizes it indirectly!
vara = 1000 # [500, 750, 1000, 1250, 1500]
if opt == 0: vara = 1000 # Enforce!
refresh = None # [100, 200, 400, 800] # Careful Usage! The function 'construct_stem' utilizes it indirectly!
paths = ['0', 'Vara_0', 'Vara_1', 'Vara_Refresh_0']
tags = ['', f'_{vara}', f'_{vara}', f'_{vara}_{refresh}']
opt_path = paths[opt] # f'./Spatial_Coupling_3D_AI_{opt_path}/'
opt_tag = tags[opt] # f'Sup_Comp{opt_tag}'
...
def construct_stem(parameter_set, parameter_set_true, para_fun, sup_comp = False, verbose = False):
    ...
    if opt in [0, 1]:
        ...
        half_activation_thresholds = {'N_N': para_fun('N_N'), 'G_G': para_fun('G_G'), 'FI_N': para_fun('FI_N'), 'G_EA': para_fun('G_EA')} # {P}_{B} = {Promoter}_{Binding_Site}
        half_repression_thresholds = {'G_N': para_fun('G_N'), 'N_G': para_fun('N_G'), 'FI_G': para_fun('FI_G'), 'N_EA': 0.5*para_fun('N_EA')} # {P}_{B} = {Promoter}_{Binding_Site}
    elif opt == 3:
        half_activation_thresholds = {'N_N': para_fun('N_N'), 'G_G': refresh, 'FI_N': refresh, 'G_EA': refresh} # {P}_{B} = {Promoter}_{Binding_Site}
        half_repression_thresholds = {'G_N': para_fun('G_N'), 'N_G': para_fun('N_G'), 'FI_G': refresh, 'N_EA': 1.5*refresh} # {P}_{B} = {Promoter}_{Binding_Site}
    else:
...
if opt in [0, 1, 2]:
    ...
    exclude = ['Initial_Pat', 'G_G', 'FI_N', 'G_EA', 'FI_G', 'N_EA']
else:
    raise RuntimeError('Oops!')
para_set, para_set_true = make_paras(P_B = (0, vara), exclude = exclude, verbose = True)
...
#%%# Inference Procedure: Simul Data Load!

if sup_comp:
    tasks = 10
    path = f'/scratch/biochemsim/ramirez/mars_projects/spatial_coupling_art_intel/data_bank/Spatial_Coupling_3D_AI_{opt_path}/'
    tag = f'Sup_Comp{opt_tag}'
    theta_set, trajectory_set = simul_data_load(tasks, path, tag)

#%%# Inference Procedure: Objective Function!

if sup_comp:
    from ObjectiveFun import ObjectiveFunOne
    from InferenceProd import InferenceProd
    safe = True
    task = int(sys.argv[1])
    task_simulations = 50
    tolerance = 0.05
    median = np.zeros(shape = (task_simulations, para_set.size), dtype = int)
    mape = np.zeros(shape = (task_simulations, para_set.size), dtype = int)
    posterior_sample_size = 200000
    posterior_samples = np.zeros(shape = (task_simulations, posterior_sample_size, para_set.size), dtype = float)
    index = 0
    while index < task_simulations:
        print(index)
        # [Preparation]
        simulations_maxi = 10
        ...
        posterior_sample_shape = tuple([posterior_sample_size])
        ...
        inference_prod_reference.verbose = False # View Raw (Simulation) Data!
        ...
        check = np.array(np.abs(observation.flatten() - objective_fun.data_objective.flatten())) > tolerance
        skip = np.any(check)
        if not skip:
            temp_median = np.array(object = np.round(vara*inference_prod_reference.synth_theta['median']), dtype = int)
            print(temp_median)
            median[index] = temp_median
            temp_mape = np.array(object = np.round(vara*inference_prod_reference.synth_theta['mape']), dtype = int)
            print(temp_mape)
            mape[index] = temp_mape
            posterior_samples[index] = np.array(inference_prod_reference.synth_posterior_samples, dtype = float)
            index += 1
        # [Restart]
        time.sleep(12)
    ...
    if safe:
        path = '/scratch/biochemsim/ramirez/mars_projects/spatial_coupling_art_intel/data_bank/Spatial_Coupling_3D_AI_Para_Est_0/'
        tag = f'Data_{vara}_{refresh}' if opt != 2 else f'Data_{vara}-500_{refresh}'
        label = f'{path}{tag}_{task}.npz'
        np.savez(file = label, median = median, mape = mape, posterior_samples = posterior_samples)
