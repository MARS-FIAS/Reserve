#####################################################
######## Spatial Coupling 3D AI Vara Refresh ########
#####################################################
...
vara = 750 # {750, 1250}
refresh = {750: {'N_N': 125, 'G_N': 50, 'N_G': 300}, 1250: {'N_N': 200, 'G_N': 50, 'N_G': 125}} # Careful Usage! The function 'construct_stem' utilizes it indirectly!
_flag = list(refresh[vara].values())
flag = ''.join(map(lambda x, y = _flag: str(x)+'-' if y.index(x) < len(y)-1 else str(x), _flag))
...
def construct_stem(parameter_set, parameter_set_true, para_fun, sup_comp = False, verbose = False):
    ...
    half_activation_thresholds = {'N_N': refresh[vara]['N_N'], 'G_G': para_fun('G_G'), 'FI_N': para_fun('FI_N'), 'G_EA': para_fun('G_EA')} # {P}_{B} = {Promoter}_{Binding_Site}
    half_repression_thresholds = {'G_N': refresh[vara]['G_N'], 'N_G': refresh[vara]['N_G'], 'FI_G': para_fun('FI_G'), 'N_EA': para_fun('N_EA')} # {P}_{B} = {Promoter}_{Binding_Site}
...
para_set, para_set_true = make_paras(P_B = (0, vara), exclude = ['Initial_Pat', 'N_N', 'G_N', 'N_G'], verbose = True)
...
if not sup_comp:
    tasks = 2
    task_simulations = 5
    path = '/media/mars-fias/MARS/MARS_Data_Bank/Art_Intel/Spatial_Coupling_3D_AI_Vara_Refresh_1/'
    tag = f"Loa_Comp_{vara}_{flag}"
    safe = True
...
if sup_comp:
    ...
    path = '/scratch/biochemsim/ramirez/mars_projects/spatial_coupling_art_intel/data_bank/Spatial_Coupling_3D_AI_Vara_Refresh_1/'
    tag = f'Sup_Comp_{vara}_{flag}'
    safe = True
...
if not sup_comp:
    tasks = 10
    path = '/media/mars-fias/MARS/MARS_Data_Bank/Art_Intel/Spatial_Coupling_3D_AI_Vara_Refresh_1/'
    tag = f'Sup_Comp_{vara}_{flag}' # 'Loa_Comp'
