...
para_set_raw = {
    'N_N': (125, (0, 1000), 1, None), 'G_G': (275, (0, 1000), 1, None),
    'G_N': (412, (0, 1000), 1, None), 'N_G': (411, (0, 1000), 1, None)
}
...
_tag = None if act == 0 else f'Act_{act-1}_Observe_{observe}_{curb.capitalize()}'
if sup_comp:
    ...
    for task in range(tasks):
        ...
        for exploration in range(task_explorations):
            if exploration == 0:
                ...
                objective_fun = ObjectiveFunPortionRule(trajectory_set, species, time_mini, time_maxi, time_unit, time_delta, simulations_maxi = 10, verbose = verbose, show = show, rules = parameter_set_rules, aim_NT = [2/5], aim_G = [3/5], L = observe)
