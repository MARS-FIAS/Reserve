####################################################
########                                    ########
######## Repression Hill Fun Variable Level ########
########                                    ########
####################################################
...
soft_props = [f'FL*kb_{P}0' for P in proteins if not all([not _ for _ in regulations[P].values()])]
...
rates.update({'FL': 1}) # Extra rate # FL := Fixed Level
...
for cove in coves:
    ...
    proto.rates['FL'] = cove
    print('N Copy Number (Fixed Level):\t', cove)
    press = BiochemSimulMule(proto, steps, trajectories, seed)
    press.meth_direct()
...
safe = True # Store Statistics?

if safe:
    # Dit
    file = open(f'{path}/Rep_Two_{seed}_{trajectories}_{steps}_{mule}', 'wb')
    exec(f'pickle.dump(dit, file)')
    file.close()
    # Dot
    file = open(f'{path}/Rep_Two_Time_{seed}_{trajectories}_{steps}_{mule}', 'wb')
...
file = open(f'{path}/Rep_Two_{seed}_{trajectories}_{steps}_{mule}', 'rb')
exec(f'dit = pickle.load(file)')
file.close()

file = open(f'{path}/Rep_Two_Time_{seed}_{trajectories}_{steps}_{mule}', 'rb')
