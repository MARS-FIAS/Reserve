...
class InferenceProd:
    ...
    def __init__(self, objective_fun, theta_set, prior, proposal = None, verbose = False):
        # Start
    ...
    def apply(self, **keywords):
        ...
        inference = inference.append_simulations(self.theta_set, self.trajectory_set, self.proposal)
    ...
    def synthesizer(self, observation, posterior_sample_shape, parameter_set_true, simulator_ready, objective_fun, **keywords):
        ...
        train_trial = keywords.get('train_trial', 0)
        simul_trials = keywords.get('simul_trials', 1)
        _time_mini = objective_fun.time_mini # Welfare purpose!
        _time_maxi = objective_fun.time_maxi # Welfare purpose!
        _time_delta = objective_fun.time_delta # Welfare purpose!
        for simul_trial in range(simul_trials):
            ...
            if simul_trial == 0:
                sap = objective_fun.data_objective.shape
                shape = (simul_trials, sap[1], sap[2], sap[3])
                data_objective = np.full(shape, np.nan)
            if self.verbose:
                objective_fun._preview_data(objective_fun.data, objective_fun.species)
                x = self.objective_fun.tau # np.arange(self.objective_fun.time_mini, self.objective_fun.time_maxi+1, self.objective_fun.time_delta)
                plt.plot(x, observation, drawstyle = 'steps-post', linestyle = '-')
                ...
                plt.grid(linestyle = '--')
                plt.show()
            data_objective[[simul_trial]] = objective_fun.data_objective
            objective_fun.time_mini = _time_mini # Welfare purpose!
            objective_fun.time_maxi = _time_maxi # Welfare purpose!
            objective_fun.time_delta = _time_delta # Welfare purpose!
            objective_fun.data = np.array(None) # Welfare purpose!
        objective_fun.data_objective = data_objective
        self.synth_objective_fun = objective_fun
        return None
