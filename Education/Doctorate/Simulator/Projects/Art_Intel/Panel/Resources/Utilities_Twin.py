...
def instate_state_tor(stem, cell_layers, layer_cells, state_tors, pat_mode = None, initial_pattern = None, blank = False, **keywords):
    if blank:
        ...
        cell_hood_dit = keywords.get('cell_hood_dit', None) # Cell Slay!
        cellulate_indices = [_[0] for _ in cell_hood_dit.items() if _[1].size != 0] # Cell Slay!
        for cell in range(cells):
            exec(f'_state_tor_{cell} = stem.initial_state.copy()')
        if initial_pattern is None:
            for cell in range(cells):
                if cell in range(cells_PRE) and cell in cellulate_indices:
                    exec(f'_state_tor_{cell}.update(state_tor_PRE)')
                elif cell in range(cells_PRE, cells_PRE + cells_EPI) and cell in cellulate_indices:
...
def instate_rate_mat(stem, cells, parameter_set, parameter_set_true, para_fun, rates_exclude, rho_mat, sigma_vet, blank = False, **keywords):
    if blank:
        ...
        # NIL [Start]
        cell_hood_dit = keywords.get('cell_hood_dit', None) # Cell Slay!
        NIL = ['ks_V', 'kd_V'] # Cell Slay!
        rate_mat_NIL = {key: 0 for key in stem.rates.keys() if key not in NIL} # Cell Slay!
        cellulate_indices = [_[0] for _ in cell_hood_dit.items() if _[1].size != 0] # Cell Slay!
        # NIL [Final]
        for cell in range(cells):
            ...
            if cell != cells - 1:
                ...
                if cell in cellulate_indices:
                    exec(f'_rate_mat_{cell}.update(rate_mat_ICM)')
                else:
                    exec(f'_rate_mat_{cell}.update(rate_mat_NIL)')
            elif cell == cells - 1:
