...
data_path = 'Spatial_Coupling_3D_AI_All_1_Relax_FC'
act = 0 # [0, 1, ...]
_observe = 0 # {0, 1, ...}
...
para_set_raw = {
    ...
    'FC_MRNA': (40, (0, 100)), 'FC': (160, (0, 400)),
    'td_F_MS': (7200, (300, 28800)), 'td_FL': (7200, (300, 28800)),
    'tau_M': (450, (30, 2*2100)), 'tau_L': (450, (30, 3*900)), 'tau_S': (450, (30, 2*2100))
}
para_set_mode = 0 # {0, 1}
...
def simulator(parameter_set, parameter_set_true, parameter_set_mode, species = ['N', 'G', 'N_MRNA', 'G_MRNA', 'NP'], steps = 10000, cells = 3, time_mini = 0, time_maxi = 48, time_unit = 'Hours', time_delta = 0.2, cell_layers = 2, layer_cells = 1, faces = 2*3, seed = None, jump_diffuse_seed = 0, sup_comp = False, verbose = False):
    ...
    pat = {'N_MRNA': (50, 0.5), 'G_MRNA': (50, 0.5), 'N': (200, 0.5), 'G': (200, 0.5), 'FC_MRNA': (para_fun('FC_MRNA'), 0.5), 'FC': (para_fun('FC'), 0.5)}
    pat_mode = 'Fish_Bind'
    initial_pattern = make_initial_pattern(pat, pat_mode, verbose, species = species, cells = cells, seed = seed)
    ...
    rate_mat = instate_rate_mat(stem, cells, parameter_set, parameter_set_true, para_fun, rates_exclude, rho_mat, sigma_vet, blank = False, kd_FM = kd_FM, kd_FL = kd_FL, kd_FS_UP = kd_FS_UP, kd_FS_DO = kd_FS_DO, ksig_C = ksig_C, ksig_M = ksig_M, ksig_L = ksig_L, ksig_S_UP = ksig_S_UP, ksig_S_DO = ksig_S_DO)
    ...
    data_inter = interpolator(simul, species, time_mini, time_maxi, time_unit, time_delta, kind = 0, sup_comp = sup_comp, verbose = verbose, err = False, fill_value = np.nan)
...
species = ['N', 'G', 'N_MRNA', 'G_MRNA', 'NP', 'FC', 'FC_MRNA', 'FM', 'FD', 'FL', 'FS_UP', 'FS_DO', 'EI', 'EA']
...
if sup_comp:
    path = f'/scratch/biochemsim/ramirez/mars_projects/spatial_coupling_art_intel/data_bank/{data_path}/' + _path
else:
    path = f'/media/mars-fias/MARS/MARS_Data_Bank/Art_Intel/{data_path}/' + _path
post = '_Posterior.pkl'
if act == 0:
    proposal = prior
else:
...
from ObjectiveFun import ObjectiveFunTwo
...
if inference_prod_activate:
    ...
    for train_trial in range(train_trials):
        # Preparation
        objective_fun_reference = ObjectiveFunTwo(trajectory_set, species, time_mini, time_maxi, time_unit, time_delta, simulations_maxi = 10, verbose = False, show = False)
        ...
        # Synthetic Data
        objective_fun = ObjectiveFunTwo(None, species, time_mini, time_maxi, time_unit, time_delta, simulations_maxi = 10, verbose = False, show = False)
