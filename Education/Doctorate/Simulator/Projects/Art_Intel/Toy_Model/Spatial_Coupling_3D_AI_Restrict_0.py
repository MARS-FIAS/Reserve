#################################################
######## Spatial Coupling 3D AI Restrict ########
#################################################
...
curb = 'alp' # {'alp', 'bet'}
restrict = {
    'alp': {'N_N': 260, 'G_G': 210, 'FI_N': 850, 'G_EA': 210, 'G_N': 120, 'N_G': 820, 'FI_G': 960, 'N_EA': 300},
    'bet': {'N_N': 85, 'G_G': 710, 'FI_N': 250, 'G_EA': 110, 'G_N': 180, 'N_G': 275, 'FI_G': 35, 'N_EA': 475}
}
...
include = {key: (value, (0.95 * value, 1.05 * value)) for key, value in restrict[curb].items()}
...
def simulator(parameter_set, parameter_set_true, species = ['N', 'G', 'N_MRNA', 'G_MRNA', 'NP', 'FED', 'EA'], steps = 10000, cells = 3, time_mini = 0, time_maxi = 48, time_unit = 'Hours', time_delta = 0.2, cell_layers = 2, layer_cells = 1, faces = 2*3, seed = None, jump_diffuse_seed = 0, sup_comp = False, verbose = False):
    ...
    state_tor_PRE = {'N': 0*500, 'G': 0*1000, 'N_MRNA': 0*250, 'G_MRNA': 0*250, 'FI': 0*800, 'FI_MRNA': 0*200, 'EI': 1*1000}
    state_tor_EPI = {'N': 0*500, 'G': 0*1000, 'N_MRNA': 0*250, 'G_MRNA': 0*250, 'FI': 0*800, 'FI_MRNA': 0*200, 'EI': 1*1000}
...
if not sup_comp:
    tasks = 2
    task_simulations = 5
    path = '/media/mars-fias/MARS/MARS_Data_Bank/Art_Intel/Spatial_Coupling_3D_AI_Restrict_0/'
    ...
    for task in range(tasks):
        theta_set = torch.zeros((task_simulations, len(para_set)))
        trajectory_set = torch.zeros((task_simulations, len(species)*len(x)*cells))
        for task_simulation in range(task_simulations):
            _theta_set = torch.reshape(torch.tensor(para_set), (1, -1))
            _trajectory_set = simulator_ready(_theta_set)
            theta_set[task_simulation] = _theta_set
            trajectory_set[task_simulation] = _trajectory_set
...
if sup_comp:
    task = int(sys.argv[1])
    task_simulations = 100
    path = '/scratch/biochemsim/ramirez/mars_projects/spatial_coupling_art_intel/data_bank/Spatial_Coupling_3D_AI_Restrict_0/'
