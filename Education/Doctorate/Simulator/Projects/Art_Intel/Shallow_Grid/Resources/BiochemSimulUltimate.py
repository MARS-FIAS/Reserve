...
class BiochemSimulSwift:
    ...
    def _ran_tor(self):
        self.ran_tor = np.random.default_rng(seed = self.seed).uniform(size = (self.steps, 3, self.cells))
    ...
    def _refresh_delta(self):
        ...
        if '_refresh_delta' not in globals():
            ...
            def __refresh_delta__(ran_tor, step, cell, alps, delta_mat):
                ...
                # _clues = np.indices(clues_boa.shape, dtype = np.uint16)
                # clues = _clues[0]
                clues = np.arange(clues_boa.size, dtype = np.uint16)
                temp = np.where(clues_boa, clues, clues.shape[0])
    ...
    def jump_diffuse_assemble(self, comm_classes_portrait):
        # Zero
        ...
        mess = ''.join(_mess)
        # print(mess)
        if self.verbose: print('i := comm_class\nj := cell\nk := neighbor')
        ...
        for comm_class in range(comm_class_tor.shape[0]):
            ...
            for cell in range(comm_class_tor.shape[1]):
                ...
                if self.verbose: print(f'\t{(comm_class, cell)}\n{np.vstack((neighbors, jump_diffuse_probabilities))}\n')
                # print(f'\t{cell}\n\t\t{neighbors}\n\t\t{jump_diffuse_probabilities}')
                comm_class_summary.update({(comm_class, cell): (neighbors, jump_diffuse_probabilities)})
            if self.verbose: print(f'{decor}\n')
        # Two
        # Last
        jump_diffuse_tor = np.full(shape = (comm_class_tor.shape[0], self.cells, self.cells), fill_value = np.nan, dtype = np.float64)
        a = np.arange(self.cells)
        if self.verbose: print(f'{decor}\n\nCells {a}')
        for comm_class in range(comm_class_tor.shape[0]):
            for cell in range(comm_class_tor.shape[1]):
                if self.verbose: print(f'\n(Comm Class, Cell) {(comm_class, cell)}')
                _jump_diffuse_tor = comm_class_tor[comm_class, cell]
                if self.verbose:
                    print(_jump_diffuse_tor)
                    print(np.flatnonzero(_jump_diffuse_tor).tolist())
                    print(comm_class_summary[(comm_class, cell)][0])
                assert np.flatnonzero(_jump_diffuse_tor).tolist() == comm_class_summary[(comm_class, cell)][0], 'Oops! Something went wrong!'
                jump_diffuse_tor[comm_class, :, cell] = _jump_diffuse_tor
        if self.verbose: print(f'\n{decor}\n')
        for comm_class in range(comm_class_tor.shape[0]):
            if self.verbose: print(f'Comm Class {comm_class}\n\n{jump_diffuse_tor[comm_class]}\n')
        # Last
    ...
    def _refresh_jump_diffuse_where(self):
        if self.verbose:
            fun = '__refresh_jump_diffuse_where__'
            arcs = []
            print(fun, '\n\t', arcs)
        if '_refresh_jump_diffuse_where' not in globals():
            global __refresh_jump_diffuse_where__
            @numba.njit
            def __refresh_jump_diffuse_where__(ran_tor, step, cell, comm_class, jump_diffuse_tor):
                cume = np.cumsum(jump_diffuse_tor[comm_class, :, cell])
                clues_boa = ran_tor[step, 2, cell] <= cume
                clues = np.arange(clues_boa.size, dtype = np.uint16)
                temp = np.where(clues_boa, clues, clues.shape[0])
                jump_diffuse_where = np.nanmin(temp)
                return jump_diffuse_where
        return __refresh_jump_diffuse_where__
            ...
            def __meth_direct__(steps, cells, step_cells, stream_epoch, stream_alps, epoch_mat, pre, state_tor, state_tor_objective, species_objective_indices, ran_tor, rate_mat, indices, delta_mat, jump_diffuse_vet, jump_diffuse_comm_classes, jump_diffuse_tor, jump_diffuse_delta_mat, epoch_halt):
                ...
                for step in range(1, steps*cells):
                    ...
                    if jump_diffuse_vet[inventory] and jump_diffuse_tor is not None:
                        ...
                        #     pass
                        comm_class = jump_diffuse_comm_classes[inventory]
                        where = _refresh_jump_diffuse_where(ran_tor, step_cell, cell, comm_class, jump_diffuse_tor)
                        ####
    ...
    def meth_direct(self, jump_diffuse_tor = None, epoch_halt_tup = (None, None)):
        ...
        global _refresh_jump_diffuse_where
        ...
        _refresh_jump_diffuse_where = self._refresh_jump_diffuse_where()
