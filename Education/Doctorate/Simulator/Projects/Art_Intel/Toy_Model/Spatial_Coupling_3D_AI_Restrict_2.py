...
curb = 'FeedBack-Off-Zero' # {'FeedBack-Off-Zero', 'Strong-Interact-Zero'} # Zero Initial FGF4!
restrict = {
    'FeedBack-Off-Zero': {'N_N': 1000, 'G_G': 1000, 'FI_N': 1000, 'G_EA': 2000, 'G_N': 1000, 'N_G': 1000, 'FI_G': 1000, 'N_EA': 2000},
    'Strong-Interact-Zero': {'N_N': 1000, 'G_G': 1000, 'FI_N': 1000, 'G_EA': 1500, 'G_N': 1000, 'N_G': 1000, 'FI_G': 1000, 'N_EA': 1500}
}
...
include = {key: (value, (value - 1000, value)) for key, value in restrict[curb].items()}
...
def simulator(parameter_set, parameter_set_true, species = ['N', 'G', 'N_MRNA', 'G_MRNA', 'NP', 'FED', 'EA'], steps = 10000, cells = 3, time_mini = 0, time_maxi = 48, time_unit = 'Hours', time_delta = 0.2, cell_layers = 2, layer_cells = 1, faces = 2*3, seed = None, jump_diffuse_seed = 0, sup_comp = False, verbose = False):
    ...
    state_tor_EPI = {'N': 1*1000, 'G': 0*1000, 'N_MRNA': 1*250, 'G_MRNA': 0*250, 'FI': 0*800, 'FI_MRNA': 0*200, 'EI': 1*1000}
...
if not sup_comp:
    tasks = 2
    task_simulations = 5
    path = '/media/mars-fias/MARS/MARS_Data_Bank/Art_Intel/Spatial_Coupling_3D_AI_Restrict_2/'
    tag = f"Loa_Comp_{curb}"
    safe = True
...
if sup_comp:
    ...
    path = '/scratch/biochemsim/ramirez/mars_projects/spatial_coupling_art_intel/data_bank/Spatial_Coupling_3D_AI_Restrict_2/'
    tag = f'Sup_Comp_{curb}'
    safe = True
