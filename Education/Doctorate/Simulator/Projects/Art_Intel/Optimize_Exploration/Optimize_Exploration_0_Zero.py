...
data_path = 'Optimize_Exploration_0_Zero' # 'Spatial_Coupling_3D_AI_All_1_Relax' # ObjectiveFunTwo
act = 0 # [0, 1, ...]
observe = 0 # {0, 1, ...}
...
from Utilities_Twin import make_para_fun, retrieve_paras
from Utilities_Twin import make_simulator_ready

para_set_raw = {
    'N_N': (235, (0, 1000), 1, None), 'G_G': (170, (0, 1000), 1, None), 'FC_N': (255, (0, 1000), 1, None), 'G_EA': (385, restrict[curb]['G_EA'], 1, None),
    'G_N': (50, (0, 1000), 1, None), 'N_G': (455, (0, 1000), 1, None), 'FC_G': (370, (0, 1000), 1, None), 'N_EA': (195, restrict[curb]['N_EA'], 1, None),
    'td_F_MS': (14795, (300, 28800), 1, None), 'td_FL': (15525, (300, 28800), 1, None),
    'tau_M': (325, (30, 2100), 1, None), 'tau_L': (425, (30, 900), 1, None), 'tau_S': (415, (30, 2100), 1, None)
}
...
def simulator(parameter_set, parameter_set_true, parameter_set_mode, species = ['N', 'G', 'N_MRNA', 'G_MRNA', 'NP'], steps = 10000, cells = 3, time_mini = 0, time_maxi = 48, time_unit = 'Hours', time_delta = 0.2, cell_layers = 2, layer_cells = 1, faces = 2*3, seed = None, jump_diffuse_seed = 0, sup_comp = False, verbose = False):
    ...
    pat = {'N_MRNA': (50, 0.5), 'G_MRNA': (50, 0.5), 'N': (200, 0.5), 'G': (200, 0.5)}
    pat_mode = 'Fish_Bind'
    ...
    for task in range(tasks):
        ...
        for exploration in range(task_explorations):
            if exploration == 0:
                ...
                objective_fun = ObjectiveFunTwo(trajectory_set, species, time_mini, time_maxi, time_unit, time_delta, simulations_maxi = 10, verbose = verbose, show = show, alpha = 1)
                objective_fun.apply(tau_mini = 24, tau_maxi = 48) # score_set
                current_meta_score = torch.tensor(objective_fun.appraise(phi = 0.5**5, psi = 1-0.5**5))
                ...
                objective_fun = ObjectiveFunTwo(trajectory_set, species, time_mini, time_maxi, time_unit, time_delta, simulations_maxi = 10, verbose = verbose, show = show, alpha = 1)
                objective_fun.apply(tau_mini = 24, tau_maxi = 48) # score_set
                explore_meta_score = torch.tensor(objective_fun.appraise(phi = 0.5**5, psi = 1-0.5**5))
