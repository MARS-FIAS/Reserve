#####################################################
######## Spatial Coupling 3D AI Signal Model ########
#####################################################
...
from Utilities import instate_state_tor
...
include = {'N': (200, (0, 1000)), 'tau_void': (210, (30, 900)), 'tau_men': (1890, (30, 2100))}
...
def instate_rate_mat(stem, cells, parameter_set, parameter_set_true, para_fun, rates_exclude, rho_mat, blank = False, **keywords):
    if blank:
        ...
        k_sig = 1*stem.rates['ke_FIE'] # 0.75*10*_rates_promoter_binding['EA'] # Intracellular -> Extracellular
        k_men = keywords.get('k_men', k_sig) # 0.75*10*_rates_promoter_binding['EA'] # Membrane
        phi_auto = 0.5 # Auto # [0, 1]
        phi_para = 1 - phi_auto # Para # [0, 1]
        psi = 1 # {Membrane, Void, Sink} # [0, 'infinity')
        psi_void = (1/3)*keywords.get('psi_void', k_men) # Void # [0, 'infinity')
        psi_sink = 0.5 # Sink # [0, 'infinity')
        psi_void_sink = (2/3)*keywords.get('psi_void', k_men) # From Void To Sink # [0, 'infinity')
        psi_sink_void = 0.5 # From Sink To Void # [0, 'infinity')
        rate_mat_V = {'kjd_FVE': psi_void, 'kjd_FSE': psi_sink*k_men, 'ke_VS': psi_void_sink, 'ke_SV': psi_sink_void*k_men}
        rate_mat_V.update({key: 0 for key in stem.rates.keys() if key not in rates_exclude})
        # ke_FIE + kjd_FIE + kjd_FIV + kjd_FIS = '1' # kjd_FE + kjd_FEV + kjd_FES = '1' # kjd_FVE + ke_VS = '1' # kjd_FSE + ke_SV = '1'
...
def simulator(parameter_set, parameter_set_true, species = ['N', 'G', 'N_MRNA', 'G_MRNA', 'NP', 'FED', 'EA'], steps = 10000, cells = 3, time_mini = 0, time_maxi = 48, time_unit = 'Hours', time_delta = 0.2, cell_layers = 2, layer_cells = 1, faces = 2*3, seed = None, jump_diffuse_seed = 0, sup_comp = False, verbose = False):
    ...
    state_tor_PRE = {'N': para_fun('N'), 'G': 0, 'N_MRNA': 0, 'G_MRNA': 0, 'NP': 0, 'FED': 0, 'EA': 0, 'EI': 1000, 'FI': 0, 'FI_MRNA': 0, 'FE': 0, 'FV': 0, 'FS': 0}
    state_tor_EPI = {'N': para_fun('N'), 'G': 0, 'N_MRNA': 0, 'G_MRNA': 0, 'NP': 0, 'FED': 0, 'EA': 0, 'EI': 1000, 'FI': 0, 'FI_MRNA': 0, 'FE': 0, 'FV': 0, 'FS': 0}
    state_tor_V = {'N': 0, 'G': 0, 'N_MRNA': 0, 'G_MRNA': 0, 'NP': 0, 'FED': 0, 'EA': 0, 'EI': 0, 'FI': 0, 'FI_MRNA': 0, 'FE': 0, 'FV': 0, 'FS': 0}
    ...
    tau_void = para_fun('tau_void')
    tau_men = para_fun('tau_men')
    rate_mat = instate_rate_mat(stem, cells, parameter_set, parameter_set_true, para_fun, rates_exclude, rho_mat, blank = False, psi_void = 1/tau_void, k_men = 1/tau_men)
...
if not sup_comp:
    tasks = 2
    task_simulations = 5
    path = '/media/mars-fias/MARS/MARS_Data_Bank/Art_Intel/Spatial_Coupling_3D_AI_Signal_Model_1_N/'
...
if sup_comp:
    ...
    path = '/scratch/biochemsim/ramirez/mars_projects/spatial_coupling_art_intel/data_bank/Spatial_Coupling_3D_AI_Signal_Model_1_N/'
