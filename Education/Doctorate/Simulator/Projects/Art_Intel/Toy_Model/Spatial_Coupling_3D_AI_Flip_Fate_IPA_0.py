######################################################
######## Spatial Coupling 3D AI Flip Fate IPA ########
######################################################
...
curb = 'alp' # {'alp', 'bet'}
restrict = {
    'alp': {'N_N': 150, 'G_G': 200, 'FI_N': 575, 'G_EA': 1000, 'G_N': 175, 'N_G': 425, 'FI_G': 500, 'N_EA': 1250},
    'bet': {'N_N': 175, 'G_G': 450, 'FI_N': 575, 'G_EA': 525, 'G_N': 50, 'N_G': 325, 'FI_G': 500, 'N_EA': 750}
}
...
def construct_stem(parameter_set, parameter_set_true, para_fun, sup_comp = False, verbose = False):
    ...
    half_activation_thresholds = {'N_N': restrict[curb]['N_N'], 'G_G': restrict[curb]['G_G'], 'FI_N': restrict[curb]['FI_N'], 'G_EA': restrict[curb]['G_EA']} # {P}_{B} = {Promoter}_{Binding_Site}
    half_repression_thresholds = {'G_N': restrict[curb]['G_N'], 'N_G': restrict[curb]['N_G'], 'FI_G': restrict[curb]['FI_G'], 'N_EA': restrict[curb]['N_EA']} # {P}_{B} = {Promoter}_{Binding_Site}
...
from Utilities import make_paras, retrieve_paras, make_para_fun
from Utilities import make_simulator_ready

include = {'Perturb': (0.5, (0, 1)), 'Posit': (0, (0, 1))}
para_set, para_set_true = make_paras(include = include, verbose = True)
...
def simulator(parameter_set, parameter_set_true, species = ['N', 'G', 'N_MRNA', 'G_MRNA', 'NP', 'FED', 'EA'], steps = 10000, cells = 3, time_mini = 0, time_maxi = 48, time_unit = 'Hours', time_delta = 0.2, cell_layers = 2, layer_cells = 1, faces = 2*3, seed = None, jump_diffuse_seed = 0, sup_comp = False, verbose = False):
    ...
    perturb = para_fun('Perturb')
    _perturb = 0.5
    posit = para_fun('Posit')
    _posit = np.array([index/(cells-1) for index in range(cells-1)])
    aim = np.max(np.argwhere(_posit <= np.array(posit)).flatten())
    ...
    state_tor_PRE = {'N': 0, 'G': 1000, 'N_MRNA': 5, 'G_MRNA': 250, 'NP': 10, 'FED': 60, 'EA': 550, 'EI': 450, 'FI': 0, 'FI_MRNA': 0, 'FE': 125, 'FV': 0, 'FS': 0}
    state_tor_EPI = {'N': 10, 'G': 20, 'N_MRNA': 225, 'G_MRNA': 5, 'NP': 425, 'FED': 60, 'EA': 550, 'EI': 450, 'FI': 20, 'FI_MRNA': 80, 'FE': 125, 'FV': 0, 'FS': 0}
    state_tor_V = {'N': 0, 'G': 0, 'N_MRNA': 0, 'G_MRNA': 0, 'NP': 0, 'FED': 0, 'EA': 0, 'EI': 0, 'FI': 0, 'FI_MRNA': 0, 'FE': 0, 'FV': 80, 'FS': 700}
    _state_tor_PRE = {key: int(value+2*(value*(perturb-_perturb))) for key, value in state_tor_PRE.items()}
    _state_tor_EPI = {key: int(value+2*(value*(perturb-_perturb))) for key, value in state_tor_EPI.items()}
    _state_tor_V = {key: int(value+2*(value*(perturb-_perturb))) for key, value in state_tor_V.items()}
    state_tors = {'state_tor_PRE': _state_tor_PRE, 'state_tor_EPI': _state_tor_EPI, 'state_tor_V': _state_tor_V}
    state_tor = instate_state_tor(stem, cell_layers, layer_cells, state_tors, blank = False)
    
    if aim == cells:
        mess = "Oops! We must not aim at modifying the void!\nThe index '{aim}' is invalid!"
        raise RuntimeError(mess)
    else:
        cells_ICM = cells - 1
        cells_EPI = cells_ICM - layer_cells
        cells_PRE = cells_ICM - cells_EPI
        _state_tor_PRE = state_tor[:, 0]
        _state_tor_EPI = state_tor[:, cells_ICM - cells_EPI]
        if aim in range(0, cells_PRE):
            state_tor[:, aim] = _state_tor_EPI
        elif aim in range(cells_ICM - cells_EPI, cells_ICM):
            state_tor[:, aim] = _state_tor_PRE
        else:
            mess = "Oops! Something went wrong with the aim index!\nWe should aim at the integer range '[0, {cells - 1}]'!"
            raise RuntimeError(mess)
...
species = ['N', 'G', 'N_MRNA', 'G_MRNA', 'NP', 'FED', 'EA', 'EI', 'FI', 'FI_MRNA', 'FE', 'FV', 'FS']
steps = 3500000
...
if not sup_comp:
    tasks = 2
    task_simulations = 5
    path = '/media/mars-fias/MARS/MARS_Data_Bank/Art_Intel/Spatial_Coupling_3D_AI_Flip_Fate_IPA_0/'
...
if sup_comp:
    ...
    path = '/scratch/biochemsim/ramirez/mars_projects/spatial_coupling_art_intel/data_bank/Spatial_Coupling_3D_AI_Flip_Fate_IPA_0/'
