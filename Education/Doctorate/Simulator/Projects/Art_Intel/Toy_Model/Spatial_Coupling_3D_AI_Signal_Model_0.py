...
include = {'FV': (750, (0, 15000)), 'tau_void': (900, (1, 2100)), 'tau_void_sink': (900, (1, 2100))}
...
def instate_rate_mat(stem, cells, parameter_set, parameter_set_true, para_fun, rates_exclude, rho_mat, blank = False, **keywords):
    if blank:
        ...
        k_men = 0.5*stem.rates['kjd_FIE'] # 0.75*10*_rates_promoter_binding['EA'] # Membrane
        phi_auto = 0.5 # Auto # [0, 1]
        phi_para = 1 - phi_auto # Para # [0, 1]
        psi = 1 # {Membrane, Void, Sink} # [0, 'infinity')
        psi_void = keywords.get('psi_void', k_men) # Void # [0, 'infinity')
        psi_sink = 1 # Sink # [0, 'infinity')
        psi_void_sink = keywords.get('psi_void_sink', k_men) # From Void To Sink # [0, 'infinity')
...
def simulator(parameter_set, parameter_set_true, species = ['N', 'G', 'N_MRNA', 'G_MRNA', 'NP', 'FED', 'EA'], steps = 10000, cells = 3, time_mini = 0, time_maxi = 48, time_unit = 'Hours', time_delta = 0.2, cell_layers = 2, layer_cells = 1, faces = 2*3, seed = None, jump_diffuse_seed = 0, sup_comp = False, verbose = False):
    ...
    tau_void_sink = para_fun('tau_void_sink')
    rate_mat = instate_rate_mat(stem, cells, parameter_set, parameter_set_true, para_fun, rates_exclude, rho_mat, blank = False, psi_void = 1/tau_void, psi_void_sink = 1/tau_void_sink)
...
if not sup_comp:
    tasks = 2
    task_simulations = 5
    path = '/media/mars-fias/MARS/MARS_Data_Bank/Art_Intel/Spatial_Coupling_3D_AI_Signal_Model_0/'
...
if sup_comp:
    ...
    path = '/scratch/biochemsim/ramirez/mars_projects/spatial_coupling_art_intel/data_bank/Spatial_Coupling_3D_AI_Signal_Model_0/'
