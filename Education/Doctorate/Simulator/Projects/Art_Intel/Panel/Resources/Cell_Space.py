...
def cell_slay(cellulate, cell_hood_dit, dot_mat, verbose = False):
    
    cell_layers = len(cellulate)
    layer_cells = max(cellulate)
    cells = dot_mat.shape[0] # cell_layers * layer_cells + 1
    _cellulate_indices = [_ for _ in range(cell_layers*layer_cells)]
    cellulate_indices = list()
    
    for index in range(len(cellulate)):
        pick = cellulate[index]
        alp = (layer_cells - pick) // 2
        bet = index * layer_cells
        _pick_lit = alp + bet
        pick_lit = [_ + _pick_lit for _ in range(pick)]
        cellulate_indices.extend(pick_lit)
        if verbose: print(f'{index} ~ {pick}\t{pick_lit}')
    
    slay_indices = [_ for _ in _cellulate_indices if _ not in cellulate_indices]
    dot_mat[slay_indices, :] = 0 # Rows
    dot_mat[:, slay_indices] = 0 # Cols
    if verbose: print(f'Slay! {slay_indices}')
    
    for cell in range(cells):
        hood = np.nonzero(dot_mat[cell])[0]
        cell_hood_dit.update({cell: hood})
        if verbose: print(f'{cell}\t{hood}')
...
def _hood_pro_vet(cells, hood):
    for cell in range(cells):
        _hood = hood[cell]
        hood_pro_vet = np.zeros(shape = (cells, ), dtype = float)
        if len(_hood) > 0:
...
def cell_neighborhood(cell_hood_dit, dot_mat, cell_layers, layer_cells, verbose = False):
    ...
    import copy # Careful! There is a bug! If the variable 'cellulate' equals '(1, )', then the variable 'cell_hoods' is wrong!
    ...
    cellulate_indices = [_[0] for _ in cell_hood_dit.items() if _[1].size != 0]
    
    # Hood 0 # Each 'hood' has its own code piece, careful!
    
    cell_hood_dit_temp.update({cells - 1: np.empty(shape = (0, ), dtype = int)}) # There is no interaction between cell class and cavity/sink class!
    ...
    cell_hood_dit_temp.update({cells - 1: np.array([_ for _ in cellulate_indices if _ < cells_PRE], int)}) # There is interaction between cell class and 'cavity'/sink class!
    cell_hood_dit_temp.update({cell: np.array([cells_ICM], dtype = int) for cell in [_ for _ in cellulate_indices if _ < cells_PRE]})
    cell_hood_dit_temp.update({cell: np.array([], dtype = int) for cell in range(cells_PRE, cells_ICM)})
    ...
    cells_sink = [cell for cell in cellulate_indices if len(_cell_hood_dit_temp[cell]) < 2*3]
    
    cell_hood_dit_temp.update({cells - 1: np.array(cells_sink, dtype = int)}) # There is interaction between cell class and cavity/'sink' class!
    cell_hood_dit_temp.update({cell: np.array([cells_ICM], dtype = int) for cell in cells_sink})
    cell_hood_dit_temp.update({cell: np.array([], dtype = int) for cell in range(cells_ICM) if cell not in cells_sink})
...
def make_rho_mat(cell_hood_dit, faces, cell_layers, layer_cells, verbose = False):
    ...
    rho_mat = np.ones((cells, 3)) # (rho_mem, rho_lumen, rho_sink)
    faces_nil = (0, 0, 0)
    
    for cell in range(cells_PRE):
        cellulate_flag = cell in cellulate_indices
        if cellulate_flag:
            faces_mem = len(cell_hood_dit[cell])
            faces_lumen = 1
            faces_sink = faces-(faces_mem+faces_lumen)
        else:
            faces_mem, faces_lumen, faces_sink = faces_nil
        rho_mem, rho_lumen, rho_sink = faces_mem/faces, faces_lumen/faces, faces_sink/faces
        rho = (rho_mem, rho_lumen, rho_sink)
        mess = 'Oops! We made a mistake! Please, check the algorithm.'
        assert (np.sum(rho) == 1 and cellulate_flag) or (np.sum(rho) == 0 and not cellulate_flag), mess
        rho_mat[cell] = rho
