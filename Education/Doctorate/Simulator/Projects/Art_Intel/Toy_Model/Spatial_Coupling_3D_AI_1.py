...
para_set, para_set_true = make_paras(P_B = (0, 2000), verbose = True)
...
def simulator(parameter_set, parameter_set_true, species = ['N', 'G', 'N_MRNA', 'G_MRNA', 'NP', 'FED', 'EA'], steps = 10000, cells = 3, time_mini = 0, time_maxi = 48, time_unit = 'Hours', time_delta = 0.2, cell_layers = 2, layer_cells = 1, faces = 2*3, seed = None, jump_diffuse_seed = 0, sup_comp = False, verbose = False):
    ...
    pat_mode = 'Harsh'
    initial_pattern = make_initial_pattern(pat, pat_mode, verbose, cell_layers = cell_layers, layer_cells = layer_cells)
    ...
    state_tor_PRE = {'N': 0*500, 'G': 1*1000, 'N_MRNA': 0*250, 'G_MRNA': 1*250, 'FI': 0*800, 'FI_MRNA': 0*200, 'EI': 1*1000}
    state_tor_EPI = {'N': 1*500, 'G': 0*1000, 'N_MRNA': 1*250, 'G_MRNA': 0*250, 'FI': 1*800, 'FI_MRNA': 1*200, 'EI': 1*1000}
...
if not sup_comp:
    tasks = 2
    task_simulations = 5
    path = '/media/mars-fias/MARS/MARS_Data_Bank/Art_Intel/Spatial_Coupling_3D_AI_1/'
    tag = 'Loa_Comp'
    safe = True
...
if sup_comp:
    task = int(sys.argv[1])
    task_simulations = 10
    path = '/scratch/biochemsim/ramirez/mars_projects/spatial_coupling_art_intel/data_bank/Spatial_Coupling_3D_AI_1/'
    tag = 'Sup_Comp'
    safe = True
