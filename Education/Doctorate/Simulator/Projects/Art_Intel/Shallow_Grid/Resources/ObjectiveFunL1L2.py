...
class ObjectiveFunPortionRule(ObjectiveFunTemplate):
    ...
    def __init__(self, data, species, time_mini = 0, time_maxi = 48, time_unit = 'Hours', time_delta = 0.2, simulations_maxi = 10, verbose = False, show = False, **keywords):
        ...
        self.aim_G = keywords.get('aim_G', [1-_ for _ in self.aim_NT])
        self.L = keywords.get('L', 1) # self.L = keywords.get('L', 2)
    ...
    def apply(self, **keywords):
        ...
        if self.L == 1:
            score_comb = np.min(data_rule_objective, 0)
        elif self.L == 2:
            score_comb = np.power(np.prod(data_rule_objective, 0), 1/len(self.rules))
        self.data_objective = score_comb.reshape(data_objective.shape)
        if self.verbose or self.show:
            for simulation in range(simulations):
                if simulation >= self.simulations_maxi:
                    break
