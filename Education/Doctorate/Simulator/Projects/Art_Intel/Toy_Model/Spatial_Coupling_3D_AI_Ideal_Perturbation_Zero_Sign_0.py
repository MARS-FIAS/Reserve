#####################################################################
######## Spatial Coupling 3D AI Ideal Perturbation Zero Sign ########
#####################################################################
...
def construct_stem(parameter_set, parameter_set_true, para_fun, sup_comp = False, verbose = False):
    ...
    k_sig = 0*0.75*10*_rates_promoter_binding['EA']
...
def simulator(parameter_set, parameter_set_true, species = ['N', 'G', 'N_MRNA', 'G_MRNA', 'NP', 'FED', 'EA'], steps = 10000, cells = 3, time_mini = 0, time_maxi = 48, time_unit = 'Hours', time_delta = 0.2, cell_layers = 2, layer_cells = 1, faces = 2*3, seed = None, jump_diffuse_seed = 0, sup_comp = False, verbose = False):
    ...
    _state_tor_PRE = {key: int(value+2*(value*(pat-_pat))) for key, value in state_tor_PRE.items()}
    _state_tor_EPI = {key: int(value+2*(value*(pat-_pat))) for key, value in state_tor_EPI.items()}
    _state_tor_V = {key: int(value+2*(value*(pat-_pat))) for key, value in state_tor_V.items()}
...
if not sup_comp:
    tasks = 2
    task_simulations = 5
    path = '/media/mars-fias/MARS/MARS_Data_Bank/Art_Intel/Spatial_Coupling_3D_AI_Ideal_Perturbation_Zero_Sign_0/'
...
if sup_comp:
    ...
    path = '/scratch/biochemsim/ramirez/mars_projects/spatial_coupling_art_intel/data_bank/Spatial_Coupling_3D_AI_Ideal_Perturbation_Zero_Sign_0/'
