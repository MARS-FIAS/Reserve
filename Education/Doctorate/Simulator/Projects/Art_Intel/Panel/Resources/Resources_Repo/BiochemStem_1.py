...
class BiochemStem:
    ...
    def _jump_diffuse_delta(self, rows, cols, bove):
        j = np.argwhere(bove).flatten().item()
    ...
    def assemble(self, show = False):
        ...
        if jump_diffuse_vet.any():
            jump_diffuse_delta = self._jump_diffuse_delta(rows = reactions, cols = species, bove = jump_diffuse_vet)
        else:
            jump_diffuse_delta = None
        self.assembly = {'species': species, 'reactions': reactions, 'prop_funs': prop_funs, 'delta_mat': delta_mat, 'jump_diffuse_vet': jump_diffuse_vet, 'jump_diffuse_delta': jump_diffuse_delta}
