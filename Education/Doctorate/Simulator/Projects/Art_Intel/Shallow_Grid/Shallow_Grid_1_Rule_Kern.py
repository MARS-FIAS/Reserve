...
data_path = 'Shallow_Grid_1_Rule_Kern'
acts = [0] # [0, 1, ...]
act = max(acts)
...
def construct_stem(parameter_set, parameter_set_true, para_fun, sup_comp = False, verbose = False):
    ...
    half_activation_thresholds = {'N_N': para_fun('N_N'), 'G_G': para_fun('G_G'), 'FC_N': 552, 'G_EA': 615} # {P}_{B} = {Promoter}_{Binding_Site}
    half_repression_thresholds = {'G_N': para_fun('G_N'), 'N_G': para_fun('N_G'), 'FC_G': 95, 'N_EA': 695} # {P}_{B} = {Promoter}_{Binding_Site}
    ...
    enzymatic = { # Reverse # [Forward | Backward]
        ...
        'rates': {'kef_EA': 1/31379, 'keb_EA': 1/1025}, # 2*Promoter_Binding/10
    ...
    phosphorylation = {
        ...
        'rates': {'kpf_NP': 1/18944, 'kpb_NP': 1/21361}, # (1|10)*Promoter_Binding
        'initial_state': {'NP': 0}
    }
...
para_set_raw = {
    'N_N': (125, (0, 1000)), 'G_G': (275, (0, 1000)),
    'G_N': (412, (0, 1000)), 'N_G': (411, (0, 1000))
}
para_set_mode = 0 # {0, 1}
...
def simulator(parameter_set, parameter_set_true, parameter_set_mode, parameter_set_rules = None, species = ['N', 'G', 'N_MRNA', 'G_MRNA', 'NP', 'FC', 'FC_MRNA', 'FM', 'FD', 'EI', 'EA'], steps = 10000, cells = 3, time_mini = 0, time_maxi = 48, time_unit = 'Hours', time_delta = 0.2, cell_layers = 2, layer_cells = 1, faces = 2*3, seed = None, sup_comp = False, verbose = False):
    ...
    for rule in parameter_set_rules:
        ...
        pat = {'N_MRNA': (117, 0.5), 'G_MRNA': (117, 0.5), 'N': (482, 0.5), 'G': (482, 0.5)}
        pat_mode = 'Fish_Bind'
...
parameter_set_rules = [0] # None
species = ['N', 'G', 'NP'] # ['N', 'G', 'N_MRNA', 'G_MRNA', 'NP', 'FC', 'FC_MRNA', 'FM', 'FD', 'EI', 'EA']
steps = 7500000 if sup_comp else 3750000
...
if act == 0:
    ...
    proposal = posterior_appraisal_selection(path, _tag, postage, verbose = True)
...
if inference_prod_activate:
    ...
    for train_trial in range(train_trials):
        # Preparation
        objective_fun_reference = ObjectiveFunPortionRule(trajectory_set, species, time_mini, time_maxi, time_unit, time_delta, simulations_maxi = 10, verbose = False, show = False, rules = parameter_set_rules, aim_NT = [4/10], aim_G = [6/10], L = _observe)
        inference_prod_reference = InferenceProd(objective_fun_reference, theta_set, prior, proposal = None)
        inference_prod_reference.apply(tau_mini = 32, tau_maxi = 48)
        # Synthetic Data
        objective_fun = ObjectiveFunPortionRule(None, species, time_mini, time_maxi, time_unit, time_delta, simulations_maxi = 10, verbose = False, show = False, rules = parameter_set_rules, aim_NT = [4/10], aim_G = [6/10], L = _observe)
...
if inference_prod_activate:
    ...
    posterior.appraisal = appraisals[where]
    posterior.parameter_set_true = parameter_set_true
    _tag = f'Act_{act}_Observe_{_observe}_{curb.capitalize()}'
    if sup_comp:
        task = int(sys.argv[1])
        postage = f'_Posterior_{task}.pkl'
    else:
        postage = '_Posterior.pkl'
    if act == 0:
        locus = path + f'Observe_{_observe}/' + _tag + postage
    else:
        locus = path + _tag + postage
