...
def instate_state_tor(stem, cell_layers, layer_cells, state_tors, blank = False):
...
def instate_rate_mat(stem, cells, parameter_set, parameter_set_true, para_fun, rates_exclude, rho_mat, blank = False):
    ...
    if blank:
        ...
        # ke_FIE + kjd_FIV + kjd_FIS = '1' # kjd_FE + kjd_FEV + kjd_FES = '1' # kjd_FVE = ? # kjd_FSE = ?
...
def make_paras(verbose = False):
    ...
    para_set = np.array([value[0]/(value[1][1]-value[1][0]) for value in para_set_true.values()])
...
def make_para_fun(parameter_set, parameter_set_true):
    ...
    def para_fun(parameter_key):
        parameter_value = parameter_set[parameter_set_index[parameter_key]] * parameter_set_true[parameter_key][1][1]
