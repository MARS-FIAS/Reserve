...
class BiochemSimulMuleRapid:
    ...
    def jump_diffuse_assemble(self, comm_classes_portrait):
        ...
        for comm_class in range(comm_class_tor.shape[0]):
            for cell in range(comm_class_tor.shape[1]):
                ...
                print(f'p = {p}')
                try:
                    _jump_diffuse_tor = np.random.default_rng(seed = self.cells*int((self.seed-1)/2)+cell+1).choice(a = a, size = self.steps*self.cells, replace = True, p = p)
                except ValueError:
    ...
    def _meth_direct(self):
        ...
        if '_meth_direct' not in globals():
            ...
            @numba.njit
            def __meth_direct__(steps, cells, step_cells, stream_epoch, stream_alps, epoch_mat, pre, state_tor, ran_tor, rate_mat, indices, delta_mat, jump_diffuse_vet, jump_diffuse_comm_classes, jump_diffuse_tor, jump_diffuse_delta_mat):
    ...
    def meth_direct(self, jump_diffuse_tor = None):
        #
        global prop_funs
        global _alps
        global _refresh_tau
        ...
        # Jump Diffuse [Final]
        #
        self.epoch_mat, self.state_tor, self.step_cells = _meth_direct(self.steps, self.cells, step_cells, stream_epoch, stream_alps, epoch_mat, pre, state_tor, ran_tor, rate_mat, indices, delta_mat, jump_diffuse_vet, jump_diffuse_comm_classes, jump_diffuse_tor, jump_diffuse_delta_mat)
        #
        return self
